...
from utils.data_loader import (
    load_denoising_n2n_train,
    load_denoising_test_mix,
    fluore_to_tensor
)
from utils.practices import (
    OneCycleScheduler,
...
class Parser(argparse.ArgumentParser):
    ...
    def __init__(self):
        ...
        self.add_argument(
            ...
            type=str,
            default='four_crop',
            choices=['four_crop', 'center_crop']
        )

        # Fixed: accept multiple integer values
        self.add_argument(
            '--noise-levels-train',
            type=int,
            nargs='+',
            default=[1, 2, 4, 8, 16],
            help='training noise levels'
        )
        ...
        self.add_argument(
            ...
            type=int,
            default=50,
            help='how many captures in each group to load'
        )
        ...
        self.add_argument(
            ...
            type=int,
            default=1,
            help='manual seed used in Tensor'
        )
        ...
        self.add_argument(
            ...
            type=str,
            default='inferno',
            help='colormap for saved images'
        )
...
# Automatically use CPU if CUDA is unavailable
device = torch.device(
    f'cuda:{args.cuda}'
...
if args.transform == 'four_crop':
    ...
    # Wide-field images may have complete noise
    # in the center-crop case
...
fixed_idx = np.random.permutation(
    len(test_loader.dataset)
)[:8]

print(f'fixed test index: {fixed_idx}')

fixed_test_noisy = torch.stack([
    test_loader.dataset[i][0]
    for i in fixed_idx
])

fixed_test_clean = torch.stack([
    test_loader.dataset[i][1]
    for i in fixed_idx
])
...
print(
    f'fixed test noisy shape: '
    f'{fixed_test_noisy.shape}'
)
...
try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (
            ...
            if args.transform == 'four_crop':
                ...
                # Fuse batch and four crops
                noisy_input = noisy_input.view(
                    -1,
                    *noisy_input.shape[2:]
                )

                noisy_target = noisy_target.view(
                    -1,
        ...
        print(
            'Epoch {} training PSNR: {:.6f}, RMSE: {:.6f}'.format(
                epoch,
                psnr,
                rmse
            )
        ...
        with torch.no_grad():
            ...
            if epoch % args.plot_epochs == 0:
                ...
                print(
                    'Epoch {}: plot test denoising '
                    '[input, denoised, clean, denoised - clean]'.format(
                        epoch
                    )
                )
                ...
                # Fixed test samples
                fixed_denoised = model(
                    fixed_test_noisy
                )

                samples = torch.cat((
                    fixed_test_noisy[:4].cpu(),
                    fixed_denoised[:4].cpu(),
                    fixed_test_clean[:4],
                    fixed_denoised[:4].cpu()
                    - fixed_test_clean[:4]
                ))
                ...
                samples = torch.cat((
                    fixed_test_noisy[4:8].cpu(),
                    fixed_denoised[4:8].cpu(),
                    fixed_test_clean[4:8],
                    fixed_denoised[4:8].cpu()
                    - fixed_test_clean[4:8]
                ))
            ...
            print(
                'Epoch {}: test PSNR: {:.6f}, RMSE: {:.6f}'.format(
                    epoch,
                    psnr,
                    rmse
                )
    ...
    print(
        'Finished training {} epochs using {} seconds'.format(
            args.epochs,
            tic2 - tic
        )
    )
    ...
    print(
        'Keyboard Interrupt captured...'
        'Saving models & training logs'
    )
